...
const BurgerMenu = ({ toggleMenu, isOpen }: BurgerMenuProps) => {
  return (
    ...
    >
      <span
        className={`bg-black block h-1 w-7 rounded-sm -translate-y-0.5 transition-all duration-500 ease-out  ${
          isOpen ? "rotate-45 translate-y-1.5" : "-translate-y-0.5"
        }  
        ${isOpen ? "bg-primary" : "bg-black"}`}
      />
      <span
        className={`bg-black block h-1 w-8 rounded-sm my-0.5 transition-all duration-400 ease-out ${
